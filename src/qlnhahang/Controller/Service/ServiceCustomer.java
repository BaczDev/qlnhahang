...
import qlnhahang.Model.ModelVoucher;
...
public class ServiceCustomer {
    ...
    public ArrayList<ModelMonAn> MenuFood(String type) throws SQLException {
        ...
        String sql = "SELECT ID_MonAn, TenMon, DonGia, HinhAnh FROM MonAn WHERE Loai=? AND TrangThai='Dang kinh doanh'";
        ...
        while (r.next()) {
            ...
            ModelMonAn data = new ModelMonAn();
            data.setId(id);
            data.setTitle(name);
            data.setValue(value);
            data.setType(type);
            data.setHinhAnh(hinhAnh);
    ...
    public ArrayList<ModelMonAn> MenuFoodOrder(String type, String orderBy) throws SQLException {
        ...
        while (r.next()) {
            ...
            if (id < 90) {
                ...
                data = new ModelMonAn(new ImageIcon(getClass().getResource("/Icons/MonAn/Unknown/unknown.jpg")), id, name, value, type);
            }
            list.add(data);
        }
        r.close();
    ...
    public ModelKhachHang getCustomer(int userID) throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, SDT, DATE_FORMAT(Ngaythamgia, '%d-%m-%Y') AS NgayTG, Doanhso, Diemtichluy FROM KhachHang WHERE ID_ND=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, userID);
        ResultSet r = p.executeQuery();
        if (r.next()) {
            ...
            String phone = r.getString("SDT");
            String date = r.getString("NgayTG");
            long sales = r.getLong("Doanhso");
            int points = r.getInt("Diemtichluy");
            data = new ModelKhachHang(id, name, phone, date, sales, points);
        }
        r.close();
        p.close();
        return data;
    ...
    public void InsertHoaDon(ModelBan table, ModelKhachHang customer) throws SQLException {
        ...
        String sql = "INSERT INTO HoaDon(ID_HoaDon,ID_KH,ID_Ban,NgayHD,TienMonAn,TienGiam,Trangthai) VALUES (?,?,?,?,0,0,'Chua thanh toan')";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idHD);
    ...
    public ModelHoaDon FindHoaDon(ModelKhachHang customer) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD, '%d-%m-%Y') AS Ngay, TienMonAn, Code_Voucher, TienGiam, Tongtien, Trangthai FROM HoaDon WHERE ID_KH=? AND Trangthai='Chua thanh toan'";
        PreparedStatement p = con.prepareStatement(sql);
    ...
    public ArrayList<ModelHoaDon> getListHD(int ID_KH) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD, '%d-%m-%Y') AS Ngay, TienMonAn, Code_Voucher, TienGiam, Tongtien, Trangthai FROM HoaDon WHERE ID_KH=? ORDER BY ID_HoaDon";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, ID_KH);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelHoaDon> getListHDOrder(int ID_KH, String order) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD,'%d-%m-%Y') AS Ngay, TienMonAn, Code_Voucher, TienGiam, Tongtien, Trangthai "
                   + "FROM HoaDon WHERE ID_KH=?";

        switch (order) {
            case "Tất cả" -> {
                // Không cần thay đổi gì, giữ nguyên câu truy vấn trên
            }
            case "Dưới 1.000.000đ" -> {
                sql += " AND Tongtien < 1000000";
            }
            case "Từ 1 đến 5.000.000đ" -> {
                sql += " AND Tongtien BETWEEN 1000000 AND 5000000";
            }
            case "Trên 5.000.000đ" -> {
                sql += " AND Tongtien > 5000000";
            }
        }

        sql += " ORDER BY ID_HoaDon";
        ...
        while (r.next()) {
            int idHoaDon = r.getInt("ID_HoaDon");
            int idKH = r.getInt("ID_KH");
            int idBan = r.getInt("ID_Ban");
            String ngayHD = r.getString("Ngay");
            int tienMonAn = r.getInt("TienMonAn");
            String code_voucher = r.getString("Code_Voucher");
            int tienGiam = r.getInt("TienGiam");
            int tongtien = r.getInt("Tongtien");
    ...
    public void exchangeVoucher(int idHoaDon, String codeVoucher) throws SQLException {
        try (CallableStatement stmt = con.prepareCall("{CALL ExchangeVoucher(?, ?)}")) {
            stmt.setInt(1, idHoaDon);
            stmt.setString(2, codeVoucher);
            stmt.execute();
        }
    }
    
    public ArrayList<ModelHoaDon> getAllHoaDonByCustomer(ModelKhachHang customer) throws SQLException {
        ArrayList<ModelHoaDon> list = new ArrayList<>();
        String sql = "SELECT * FROM HoaDon WHERE ID_KH = ? AND TrangThai = 'Chua thanh toan'";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, customer.getID_KH());
        ResultSet r = p.executeQuery();
        while (r.next()) {
            int idHD = r.getInt("ID_HD");
            int idKH = r.getInt("ID_KH");
            int idBan = r.getInt("ID_Ban");
            String ngayLap = r.getString("NgayLap");
            String trangThai = r.getString("TrangThai");
            // Truyền các giá trị mặc định cho các trường số (0) nếu không dùng
            ModelHoaDon hd = new ModelHoaDon(idHD, idKH, idBan, ngayLap, 0, 0, 0);
            // Set trạng thái nếu cần
            // hd.setTrangthai(trangThai); // Nếu có setter, hoặc truyền vào constructor đầy đủ nếu cần
            list.add(hd);
        }
        r.close();
        p.close();
