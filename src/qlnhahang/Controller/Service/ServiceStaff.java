...
import qlnhahang.Model.ModelCTNK;
import qlnhahang.Model.ModelCTXK;
import qlnhahang.Model.ModelHoaDon;
import qlnhahang.Model.ModelKhachHang;
import qlnhahang.Model.ModelKho;
import qlnhahang.Model.ModelNguyenLieu;
...
public class ServiceStaff {
    ...
    public int getTongtienNK() throws SQLException {
        ...
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, sdf.format(new Date()));
        ResultSet r = p.executeQuery();
        if (r.next()) {
            tongtien = r.getInt(1);
        }
    ...
    public void InsertPNK_CTNK(ModelPNK pnk, ArrayList<ModelKho> list) throws SQLException {
        ...
        for (ModelKho data : list) {
            if (data.getSlTon() > 0) {
                String sql_ct = "INSERT INTO CTNK(ID_NK,ID_NL,SoLuong) VALUES (?,?,?)";
    ...
    public void InsertPXK_CTXK(ModelPXK pxk, ArrayList<ModelKho> list) throws SQLException {
        ...
        for (ModelKho data : list) {
            if (data.getSlTon() > 0) {
                String sql_ct = "INSERT INTO CTXK(ID_XK,ID_NL,SoLuong) VALUES (?,?,?)";
                PreparedStatement p_ct = con.prepareStatement(sql_ct);
    ...
    public ArrayList<ModelKhachHang> MenuKH() throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, SDT, DATE_FORMAT(Ngaythamgia, '%d-%m-%Y') AS NgayTG, DoanhSo, DiemTichLuy FROM KhachHang";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            int id_kh = r.getInt("ID_KH");
            String name = r.getString("TenKH");
            String phone = r.getString("SDT");
            String dateJoin = r.getString("NgayTG");
            long sales = r.getLong("DoanhSo");
            int points = r.getInt("DiemTichLuy");
            ModelKhachHang data = new ModelKhachHang(id_kh, name, phone, dateJoin, sales, points);
            list.add(data);
        }
        r.close();
        p.close();
