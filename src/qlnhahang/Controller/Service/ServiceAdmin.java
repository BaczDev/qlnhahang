...
public class ServiceAdmin {
    ...
    public ArrayList<ModelHoaDon> getListHDIn(String txt) throws SQLException {
        ...
        if (txt.equals("Tất cả")) {
            ...
            sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD, '%d-%m-%Y') as Ngay, Tienmonan, Tiengiam, Tongtien FROM HoaDon WHERE DATE(NgayHD) = CURDATE()";
        } else if (txt.equals("Tháng này")) {
            sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD, '%d-%m-%Y') as Ngay, Tienmonan, Tiengiam, Tongtien FROM HoaDon WHERE MONTH(NgayHD) = MONTH(CURDATE()) AND YEAR(NgayHD) = YEAR(CURDATE())";
        } else if (txt.equals("Năm này")) {
            sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, DATE_FORMAT(NgayHD, '%d-%m-%Y') as Ngay, Tienmonan, Tiengiam, Tongtien FROM HoaDon WHERE YEAR(NgayHD) = YEAR(CURDATE())";
        }
    ...
    public int getPreMonthRevenueHD() throws SQLException {
        int Pre_revenue = 0;  
        String sql = "SELECT SUM(Tongtien) FROM HoaDon WHERE MONTH(NgayHD) = MONTH(CURDATE()) - 1 AND YEAR(NgayHD) = YEAR(CURDATE())";
    ...
    public ArrayList<ModelPNK> getListPNKIn(String txt) throws SQLException {
        ...
        if (txt.equals("Tất cả")) {
            ...
            sql = "SELECT ID_NK, ID_NV, DATE_FORMAT(NgayNK, '%d-%m-%Y') AS Ngay, Tongtien FROM PhieuNK WHERE DATE(NgayNK) = CURDATE() ORDER BY ID_NK";
        } else if (txt.equals("Tháng này")) {
            sql = "SELECT ID_NK, ID_NV, DATE_FORMAT(NgayNK, '%d-%m-%Y') AS Ngay, Tongtien FROM PhieuNK WHERE MONTH(NgayNK) = MONTH(CURDATE()) AND YEAR(NgayNK) = YEAR(CURDATE()) ORDER BY ID_NK";
        } else if (txt.equals("Năm này")) {
            sql = "SELECT ID_NK, ID_NV, DATE_FORMAT(NgayNK, '%d-%m-%Y') AS Ngay, Tongtien FROM PhieuNK WHERE YEAR(NgayNK) = YEAR(CURDATE()) ORDER BY ID_NK";
        }
    ...
    public int getCostNK(String filter) throws SQLException {
        ...
        if (filter.equals("Hôm nay")) {
            sql += " WHERE DATE(NgayNK) = CURDATE()";
        } else if (filter.equals("Tháng này")) {
            sql += " WHERE MONTH(NgayNK) = MONTH(CURDATE()) AND YEAR(NgayNK) = YEAR(CURDATE())";
        } else if (filter.equals("Năm này")) {
            sql += " WHERE YEAR(NgayNK) = YEAR(CURDATE())";
        }
    ...
    public ArrayList<ModelMonAn> getMenuFood() throws SQLException {
        ...
        while (r.next()) {
            ...
            ImageIcon icon;
            if (id < 90) {
                String path = "/Icons/Food/" + type + "/" + id + ".jpg";
                java.net.URL location = getClass().getResource(path);
                if (location != null) {
                    icon = new ImageIcon(location);
                } else {
                    icon = new ImageIcon(getClass().getResource("/Icons/MonAn/Unknown/unknown.jpg"));
                }
            } else {
                icon = new ImageIcon(getClass().getResource("/Icons/MonAn/Unknown/unknown.jpg"));
            }
            data = new ModelMonAn(icon, id, name, value, type, state);
            list.add(data);
        }
        r.close();
        p.close();
    ...
    public ModelMonAn getMonAnById(int id) throws SQLException {
        String sql = "SELECT ID_MonAn, TenMon, DonGia, HinhAnh, Loai, TrangThai FROM MonAn WHERE ID_MonAn = ?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, id);
        ResultSet r = p.executeQuery();
        ModelMonAn data = null;
        if (r.next()) {
            data = new ModelMonAn();
            data.setId(r.getInt("ID_MonAn"));
            data.setTitle(r.getString("TenMon"));
            data.setValue(r.getInt("DonGia"));
            data.setHinhAnh(r.getString("HinhAnh"));
            data.setType(r.getString("Loai"));
            data.setState(r.getString("TrangThai"));
    ...
    public void insertMA(ModelMonAn data) throws SQLException {
        String sql = "INSERT INTO MonAn(ID_MonAn, TenMon, Dongia, Loai, TrangThai, HinhAnh) VALUES (?, ?, ?, ?, 'Dang kinh doanh', ?)";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, data.getId());
        p.setString(2, data.getTitle());
        p.setInt(3, data.getValue());
        p.setString(4, data.getType());
        p.setString(5, data.getHinhAnh() != null ? data.getHinhAnh() : "");
        p.execute();
        p.close();
    }
    ...
    public void UpdateMonAn(ModelMonAn data) throws SQLException {
        String sql = "UPDATE MonAn SET TenMon = ?, Dongia = ?, Loai = ?, HinhAnh = ? WHERE ID_MonAn = ?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setString(1, data.getTitle());
        p.setInt(2, data.getValue());
        p.setString(3, data.getType());
        p.setString(4, data.getHinhAnh() != null ? data.getHinhAnh() : "");
