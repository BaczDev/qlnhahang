...
public class InsertPXK_Form extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        tableCTNK = new qlnhahang.View.Swing.Table();
        lbDate = new javax.swing.JLabel();
        jSeparator1 = new javax.swing.JSeparator();
        lbmaXK = new javax.swing.JLabel();
        txtmaXK = new qlnhahang.View.Swing.MyTextField();
    ...
    try {
        for (ModelKho data : list) {
            if (data.getSlTon() > 0) {
                // Lấy số lượng tồn thực tế trong kho
                int slTonKho = 0;
                for (ModelKho kho : service.MenuKhoNL()) {
                    if (kho.getIdNL() == data.getIdNL()) {
                        slTonKho = kho.getSlTon();
                        break;
                    }
                }
                if (data.getSlTon() > slTonKho) {
                    enough = false;
                    tenNL = data.getTenNL();
                    break;
                }
    ...
    if (!enough) {
        // Hiện cảnh báo không đủ số lượng
        qlnhahang.View.Dialog.MS_Warning ms = new qlnhahang.View.Dialog.MS_Warning(null, true);
        ms.WarningNotEnoughStock(tenNL);
        return;
    }
    ...
    private qlnhahang.View.Swing.Table tableCTNK;
    ...
    private qlnhahang.View.Swing.MyTextField txtmaXK;
