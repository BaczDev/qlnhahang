...
public class ModelMonAn {
    ...
    // Cập nhật getIcon() để truyền type
    public Icon getIcon() {
        if (icon == null && hinhAnh != null) {
            loadIconFromHinhAnh(type); // Sử dụng type từ thuộc tính của ModelMonAn
        }
        return icon;
    }
    ...
    public void loadIconFromHinhAnh(String type) {
        if (hinhAnh != null && !hinhAnh.isEmpty() && type != null) {
            String path = "/Icons/MonAn/" + type + "/" + hinhAnh;
            ...
            if (imgURL != null) {
                try {
                    ...
                    this.icon = new ImageIcon(getClass().getResource("/Icons/MonAn/Unknown/unknown.jpg"));
                }
            } else {
